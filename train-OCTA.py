...
import numpy as np
import csv
from  datasets import datasetMT_3In,datasetMT_3In_zeiss
import random
...
def testSeg(pred,gt,imgName,image,name='FAZ'):
    AUC = 0
    mask_pred_ori = pred
    true_mask = gt
...
def test_(model,dataloader,task, isSave = True):
    ...
    with torch.no_grad():
         for image, junction_map, junction_det, FAZ, vessels in dataloader:
            ...
            pred_FAZ,pred_Vess, mask_pred_ori_map, mask_pred_ori_det = model(input_1,input_2,input_3)

            imgName = dataloader.dataset.getFileName()
            pred_FAZ = pred_FAZ.squeeze().cpu().numpy()
            labels_FAZ = FAZ.squeeze().numpy()
            ...
            AUC, DC, Acc, Sen, JS = testSeg(pred_FAZ, labels_FAZ, imgName,image)

            funcMtx_FAZ['AUC'].append(AUC)
            funcMtx_FAZ['DC'].append(DC)
            funcMtx_FAZ['Acc'].append(Acc*100)
            funcMtx_FAZ['Sen'].append(Sen) 
            funcMtx_FAZ['JS'].append(JS)
            ...
            print(imgName)
            cv2.imwrite(os.path.join(pathJ,'img_gt_'+imgName),0.6*image+0.7*BF_gt_cir+0.7*CR_gt_cir)
            cv2.imwrite(os.path.join(pathJ,'img_pred_'+imgName),0.6*image+0.7*BF_cir+0.7*CR_cir)

    length = len(dataloader.dataset)
    for k in funcMtx_FAZ.keys():
        if  'DC' in k  or 'JS' in k or 'Acc' in k:
            viz.plot('FAZ_'+k, np.mean(funcMtx_FAZ[k]))
    ...
    print('FAZ - Acc, JS, DC: %04f+%04f, %04f+%04f, %04f+%04f'%(np.mean(funcMtx_FAZ['Acc']), np.std(funcMtx_FAZ['Acc']),\
            np.mean(funcMtx_FAZ['JS']), np.std(funcMtx_FAZ['JS']), np.mean(funcMtx_FAZ['DC']), np.std(funcMtx_FAZ['DC'])))
...
def train_(model,dataloader_train,dataloader_test):
    ...
    for epoch in range(Config3In.num_epochs):
        ...
        for image, junction_map, junction_det, FAZ, vessels in dataloader_train:
            ...
            vessel_w = vessels[0].to(device)
            vessel_d = vessels[1].to(device)
            vessel_s = vessels[2].to(device)

            optimizer.zero_grad()
            outputs_FAZ, outputs_vessel, outputs_map, outputs_det = model(input_1,input_2,input_3)

            show_image = input_1* 255.
            viz.img(name='images', img_=show_image[0, :, :, :])
            viz.img(name='labels_FAZ', img_=labels_FAZ[0, :, :, :])
            viz.img(name='prediction_FAZ', img_=outputs_FAZ[0, :, :, :])
            ...
            loss_faz = criterion_bce(outputs_FAZ, labels_FAZ)

            loss_vessel = criterion_bce(outputs_vessel, vessel_w)

            train_loss = [loss_faz, loss_vessel, loss_junc]
            loss = sum([lambda_weight[i, index] * train_loss[i] for i in range(3)])
            cost[0] = train_loss[0].item()
            cost[1] = train_loss[1].item()
            cost[2] = train_loss[2].item()
            avg_cost[index, :3] += cost[:3] / train_batch
            ...
            viz.plot('loss_faz', loss_faz.item())
        ...
        if bool(epoch%3)  is False :
            ...
            metrix_FAZ, metrix_vessel, metrix_junc = metrix
            model.train(mode=True)

        if np.mean(metrix_FAZ['DC']) > best_metrix_FAZ and epoch >300:
            save_path = os.path.join(save_dir, 'state-{}-FAZ-{}.pth'.format(epoch + 1,np.mean(metrix_FAZ['DC'])))
            best_metrix_FAZ = np.mean(metrix_FAZ['DC'])
...
if __name__ =='__main__':
    ...
    #dataloader
    dataTrain = datasetMT_3In(root = Config3In.dataPath,sigma=Config3In.sigma,size=Config3In.gridSize, isTraining=True, imgSize = Config3In.imgSize)
    dataloader_Train = DataLoader(dataTrain, batch_size=Config3In.batch_size,shuffle=True, worker_init_fn=worker_init_fn)

    dataTest = datasetMT_3In(root = Config3In.dataPath,sigma=Config3In.sigma,size=Config3In.gridSize, isTraining=False, imgSize = Config3In.imgSize)
